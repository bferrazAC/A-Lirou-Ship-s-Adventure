...
namespace GeneratedGeometryPipeline
{
    ...
    public class TerrainProcessor : ContentProcessor<Texture2DContent, ModelContent>
    {
        ...
        const float terrainScale = 100;
        const float terrainBumpiness = 640;
        const float texCoordScale = 0.15f;
        const string terrainTexture = "florest.jpg";
        ...
        {
            MeshBuilder builder = MeshBuilder.StartMesh("terrain2");
            ...
            //float[, ,] MapPositions = new float[heightfield.Height, heightfield.Width, terrainBumpiness];
            ...
            for (int y = 0; y < heightfield.Height; y++)
            {
                for (int x = 0; x < heightfield.Width; x++)
                {
                    ...
                    //MapPositions[(int)position.Z, (int)position.X, (int)position.Y] = 1;
                    //Console.WriteLine(" [{0}][{1}][{2}] = [{3}]" +position.Z + position.X +position.Y + MapPositions[(int)position.Z, (int)position.X, (int)position.Y]);
                }
            }
            ...
            ModelContent model;
            model = context.Convert<MeshContent, ModelContent>(terrainMesh,
                                                              "ModelProcessor");

            model.Tag = new Ship.TerrainInfoContent(heightfield, terrainScale,
                terrainBumpiness);
